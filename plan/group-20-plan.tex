...
The storage unit will then be  brought to the user at a user specified location. The storage unit will remain in that location as needed. When the user wishes for the storage unit to be returned FInDO will proceed to pick up the unit again and take it back to the  storage area.
...
\begin{enumerate}
\item Milestone: User can activate bot from app or by voice, instructing it which item to deliver and where to.
\item To be completed by Demo 3 (March 9)
...
We are currently unsure if we are able to get the bot to learn the environment and then navigate to desired locations by itself. Should this turn out to be too difficult, we plan to have the user teach the bot fixed paths by moving it with the app.

\textbf{Mobile App}

From our preliminary research, it seems that TurtleBot-Android communication could be problematic. Should we not be able to get it working, we will try to implement a web app instead as there seems to be greater support for this.
